(function () {
    ...
    function getRatingColor(value, isPercent) {
        ...
        // Корректировка порогов для процентов Томатов и 10-бальной шкалы IMDb/TMDB
    ...
    // Функция фонового получения IMDb и Rotten Tomatoes (через OMDB API зеркало)
    function getExtendedRatings(item, callback) {
        ...
        if (!imdbId) { 
            // Фолбэк на стандартные данные Лампы, если нет ID
        ...
        reqIds.silent(url, function(resIds) {
            ...
            // Запрос к агрегатору для получения томатов и точного IMDb
            ...
            reqOmdb.silent(omdbUrl, function (data) {
                ...
                } else if (data.rotten_tomatoes) {
                    rt_c = parseInt(data.rotten_tomatoes) || 0;
                    rt_f = parseInt(data.rotten_tomatoes_audience) || 0;
                }
    ...
    // Режим МИКС (Внутри карточки в середине фильма) - СТРОГИЙ ПОРЯДОК
    ...
    // Режим РАЗДЕЛЬНО (На обложках в начальных закладках) - КРАСИВЫЕ ПЛАШКИ ДРУГ ПОД ДРУГОМ
